...
func main() {
	...
	enabled := true
	rule, err := session.AutoModerationRuleCreate(*GuildID, &dgo.AutoModerationRule{
		Name:        "Auto Moderation example",
		EventType:   dgo.AutoModerationEventMessageSend,
		TriggerType: dgo.AutoModerationEventTriggerKeyword,
		TriggerMetadata: &dgo.AutoModerationTriggerMetadata{
			KeywordFilter: []string{"*cat*"},
			RegexPatterns: []string{"(c|b)at"},
		},

		Enabled: &enabled,
		Actions: []dgo.AutoModerationAction{
			{Type: dgo.AutoModerationRuleActionBlockMessage},
		},
	})
	if err != nil {
		panic(err)
	...
	session.AddHandlerOnce(func(s *dgo.Session, e *dgo.AutoModerationActionExecution) {
		_, err = session.AutoModerationRuleEdit(*GuildID, rule.ID, &dgo.AutoModerationRule{
			TriggerMetadata: &dgo.AutoModerationTriggerMetadata{
				KeywordFilter: []string{"cat"},
			},
			Actions: []dgo.AutoModerationAction{
				{Type: dgo.AutoModerationRuleActionTimeout, Metadata: &dgo.AutoModerationActionMetadata{Duration: 60}},
				{Type: dgo.AutoModerationRuleActionSendAlertMessage, Metadata: &dgo.AutoModerationActionMetadata{
		...
		session.AddHandler(func(s *dgo.Session, e *dgo.AutoModerationActionExecution) {
			action := "unknown"
			switch e.Action.Type {
			case dgo.AutoModerationRuleActionBlockMessage:
				action = "block message"
			case dgo.AutoModerationRuleActionSendAlertMessage:
				action = "send alert message into <#" + e.Action.Metadata.ChannelID + ">"
			case dgo.AutoModerationRuleActionTimeout:
				action = "timeout"
			}
