...
var (
	...
	dmPermission                   = false
	defaultMemberPermissions int64 = dgo.PermissionManageGuild
	...
	commands = []*dgo.ApplicationCommand{
			...
			Options: []*dgo.ApplicationCommandOption{
				{
					...
					NameLocalizations: map[dgo.Locale]string{
						dgo.ChineseCN: "一个本地化的选项",
					},
					DescriptionLocalizations: map[dgo.Locale]string{
						dgo.ChineseCN: "这是一个本地化的选项",
					},
		...
		},
		{
			...
			Options: []*dgo.ApplicationCommandOption{
				...
				{
					Type:        dgo.ApplicationCommandOptionNumber,
					Name:        "number-option",
	...
	commandHandlers = map[string]func(s *dgo.Session, i *dgo.InteractionCreate){
		"basic-command": func(s *dgo.Session, i *dgo.InteractionCreate) {
		...
		"basic-command-with-files": func(s *dgo.Session, i *dgo.InteractionCreate) {
			s.InteractionRespond(i.Interaction, &dgo.InteractionResponse{
				Type: dgo.InteractionResponseChannelMessageWithSource,
				Data: &dgo.InteractionResponseData{
		...
		"localized-command": func(s *dgo.Session, i *dgo.InteractionCreate) {
			responses := map[dgo.Locale]string{
				dgo.ChineseCN: "你好！ 这是一个本地化的命令",
			}
		...
		"options": func(s *dgo.Session, i *dgo.InteractionCreate) {
			// Access options in the order provided by the user.
			options := i.ApplicationCommandData().Options

			// Or convert the slice into a map
			optionMap := make(map[string]*dgo.ApplicationCommandInteractionDataOption, len(options))
		...
		"permission-overview": func(s *dgo.Session, i *dgo.InteractionCreate) {
			perms, err := s.ApplicationCommandPermissions(s.State.User.ID, i.GuildID, i.ApplicationCommandData().ID)

			var restError *dgo.RESTError
			if errors.As(err, &restError) && restError.Message != nil && restError.Message.Code == dgo.ErrCodeUnknownApplicationCommandPermissions {
				s.InteractionRespond(i.Interaction, &dgo.InteractionResponse{
					Type: dgo.InteractionResponseChannelMessageWithSource,
					Data: &dgo.InteractionResponseData{
			...
			for _, o := range perms.Permissions {
				...
				switch o.Type {
				case dgo.ApplicationCommandPermissionTypeUser:
					users += fmt.Sprintf(format, emoji, "<@!"+o.ID+">")
				case dgo.ApplicationCommandPermissionTypeChannel:
					allChannels, _ := dgo.GuildAllChannelsID(i.GuildID)
				...
				case dgo.ApplicationCommandPermissionTypeRole:
					if o.ID == i.GuildID {
		...
		"subcommands": func(s *dgo.Session, i *dgo.InteractionCreate) {
			options := i.ApplicationCommandData().Options
			content := ""
		...
		"responses": func(s *dgo.Session, i *dgo.InteractionCreate) {
			...
			case int64(dgo.InteractionResponseChannelMessageWithSource):
				content =
		...
		"followups": func(s *dgo.Session, i *dgo.InteractionCreate) {
			...
			s.FollowupMessageEdit(i.Interaction, msg.ID, &dgo.WebhookEdit{
				Content: &content,
			})
