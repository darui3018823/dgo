...
var ErrNilState = errors.New("state not instantiated, please use dgo.New() or assign Session.State")
...
//
//	   _, err := dgo.Session.State.Guild(guildID)
//		  isInGuild := err == nil
...
func (s *State) OnInterface(se *Session, i interface{}) (err error) {
	...
	case *ChannelUpdate:
		if s.TrackChannels {
			old, oldErr := s.Channel(t.ID)
			if oldErr == nil {
				oldCopy := *old
				t.BeforeUpdate = &oldCopy
			}
			_ = s.ChannelAdd(t.Channel)
		}
	case *ChannelDelete:
		if s.TrackChannels {
